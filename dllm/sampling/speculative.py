...
``SelfSpecBackend``. ``TopologySelfSpecBackend`` is the small reference
implementation for :class:`dllm.models.DiffusionTransformer`.
"""
...
class TopologySelfSpecBackend:
    ...
    def draft_logits(
        self, block_ids: torch.Tensor, cache: KVCache
    ) -> torch.Tensor:
    ...
    def causal_verify(
        ...
        if cache.group_ids is None or cache.length == 0:
            first_group = torch.zeros(
                batch, dtype=torch.long, device=block_ids.device
            )
        else:
